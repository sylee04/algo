...
class Solution:
    def maxNumberOfBalloons(self, text: str) -> int:
        ...
        for _ in range(len(text)//7):
            for s in 'balloon':
                if s in list1:
                    ret.append(list1.pop(list1.index(s)))
#                    list1.remove(s)
                else : 
                    stop = 1
                    break
...
#    def maxNumberOfBalloons(self, text: str) -> int:
##        list1 = list(text)
#        dic1 = {k:v for k,v in enumerate(text)}
#        dic2 = {v:k for k,v in enumerate(text)}
#        ret = []
#        stop = 0
#        for _ in range(len(text)//7):
#            for s in 'balloon':
